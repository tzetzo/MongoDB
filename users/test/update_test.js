...
describe("Updating records", () => {
  ...
  beforeEach(done => {
    user = new User({ name: "John", likes: 0 }); //mongoose creates the _id of the document here
    user.save().then(() => done());
  });
  ...
  it("should increment by 1 a field value of multiple documents", done => {
    User.updateMany({ name: "John" }, { $inc: { likes: 3 } })   //Update Operator --> https://docs.mongodb.com/manual/reference/operator/update/
      .then(() => User.find({ name: "John" }))
      .then(users => {
        assert(users[0].likes === 3);
        done();
      });
  });
});
